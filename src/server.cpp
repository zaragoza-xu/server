...
Server::Server(asio::io_context &context, int port,
  ...
  if (!battle_config_complete(state->battleConfig)) {
    state->battleConfig = battle_config_loader::load_default_battle_config();
  }
}
...
int Server::get_state_status(const Protocol::GetStateStatusReq &req,
                             Protocol::GetStateStatusRsp &rsp) {
  rsp = Protocol::GetStateStatusRsp{};
  std::shared_ptr<User> member;
  std::shared_ptr<Room> room;
  {
    std::scoped_lock lock(state->usersMutex, state->roomsMutex);
    const auto userIt = state->users.find(req.uid);
    if (userIt == state->users.end() || !userIt->second) {
      return Protocol::SERVICE_FAIL | Protocol::NOT_FOUND;
    }
    ...
    const int room_id = member->get_room_id();
    const auto roomIt = state->rooms.find(room_id);
    if (roomIt == state->rooms.end() || !roomIt->second) {
      rsp.roomId = -1;
      return Protocol::SERVICE_SUCCESS;
    }

    room = roomIt->second;
    rsp = room->get_state_status_snapshot();
    rsp.online = true;
  }
  return Protocol::SERVICE_SUCCESS;
}
