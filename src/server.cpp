...
int Server::leave_room(const Protocol::LeaveRoomReq &req,
  ...
  push.type = static_cast<int>(Protocol::HomeRequestType::LEAVE_ROOM);
  push.pushMessages = {};
...
int Server::set_ready(const Protocol::SetReadyReq &req,
  ...
  Protocol::LongEnvelope push;
  push.type = static_cast<int>(Protocol::HomeRequestType::SET_READY);
  push.pushMessages = {};
  push.data =
      json{{"uid", req.uid}, {"ready", req.ready}, {"roomInfo", rsp.roomInfo}};
  broadcast_to_members(member_uids, push, req.uid);
  return Protocol::SERVICE_SUCCESS;
}
